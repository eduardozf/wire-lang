...
`direction`, `crossings`, `layout`, `orientation`, `anchor`, and net `style` are
honored today. By default (`crossings=gap`) wires that cross without a junction
are left simply overlapping; `crossings=hop` opts in to a small hop at each such
crossing. `layout=bus-rail` redraws the schematic as a block diagram between a top
supply rail and a bottom ground rail, color-coding nets by family and bundling
grouped signals into bus trunks (power and buses are inferred from net names and
...
orientation=vertical` draws a vertical resistor in a left-to-right flow.
`anchor=center` shifts a component toward the rails it connects to, so a part
wired mostly upward rises toward the top rail and an evenly-wired part stays
centered. `side` (and `group` layout) are accepted and validated but not yet
